...
impl Render for RootPlaceholder {
    fn render(&mut self, _window: &mut Window, _cx: &mut Context<Self>) -> impl IntoElement {
        div().child("picoapp (gpui-kit skeleton)")
    }
}
...
pub fn run_ui(py: Python<'_>, _inputs: &[Input], _callback: Callback) -> PyResult<()> {
    setup_logging();
    ...
    py.allow_threads(|| {
        ...
        gpui_kit::application().run(move |cx: &mut App| {
            gpui_kit::init(cx);
            open_window(window_options(), cx, |_window, cx| {
                cx.new(|_cx| RootPlaceholder)
            })
