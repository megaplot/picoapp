...
- Effective `git diff` against `main`: When working on a feature branch and moving code around multiple times, watch the effective diff against `main`. We don't want to accumulate accidental diffs from moving things around and making tiny random changes (whitespace, reflowed comments, reordered imports/items, renamed locals, changed formatting of untouched lines). Any change in the diff against `main` must be intentional -- for every hunk that touches pre-existing code ask: *is this required by the feature?* If not, it should be reverted.
...
When introducing a type along with a list of its possible runtime values, don't repeat the values in both places -- derive one from the other, or generate both from one definition:

```rust
// Avoid
const MODES: [&str; 3] = ["box", "hann", "hamming"];
enum Mode { Box, Hann, Hamming }

// Use: one definition (e.g. a `strum`-style derive, or a single `match` that maps
// between them) and derive the list/parse/display from it
```
